...
export interface XbRenderBlock {
  type: string,
  level: number,
  content?: string,
  children: XbRenderBlock[],
  lines?: number[],
...
export interface XbMarkdownParseOptions {
  raw: boolean
}
...
class XbMarkdownParse {
  ...
  tokens: [];
  renderList: XbRenderBlock[];

  STYLE_PREFIX = 'xb-rich-text-';


  parse(md: string, options: XbMarkdownParseOptions = {
    raw: false
  }) {
   ...
   * @param inlineToken
   */
  dealInlineContent(token: any, index: number, options: any) {
    if (token.type === 'text') {
      if (this.tmp.length <= 0) {   //0级dom
   ...
   * @param block
   */
  easySinglePush(block: XbRenderBlock) {
   ...
   * h1、h2....
   */
  heading_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * ul
   */
  bullet_list_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * li
   */
  list_item_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * ol
   */
  ordered_list_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * p
   */
  paragraph_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
  ...
  thead_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
  ...
  tbody_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
  ...
  tr_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
  ...
  th_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
  ...
  td_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * Bold（粗体）
   */
  strong_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * Italicize（斜体）
   */
  em_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * Strikethrough（删除线）
   */
  del_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * <ins>
   */
  ins_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * <mark>
   */
  mark_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   */
  softbreak(token) {
    const softBreakBlock: XbRenderBlock = {
      ...token,
      type: 'text',
      content: '',
      children: [],
   ...
   */
  abbr_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
  ...
  footnote_block_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
  ...
  footnote_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
   ...
   * Definition lists
   */
  dl_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
  ...
  dd_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
  ...
  dt_open(token: any, index: number, options) {
    this.commonOpenDeal(token, index, {
      ...options,
